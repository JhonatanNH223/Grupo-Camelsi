...
class Usuarios {
    ...
    constructor() {
        this.apiUrl = 'usuarios/api-usuarios.php';
        this.userLevel = nivelphp; //Nivel del usuario al iniciar secion
        this.id_user = id_userphp; //Id del usuario
    ...
    attachEventListeners() {
        ...
        this.elements.checkPassword.addEventListener('change', function () {
            ...
            if (this.checked) {
                ...
                passwordInput.value = ''; // Borra el valor
    ...
    renderTabla(data) {
        ...
        ${(this.userLevel == 'ADMIN' && fila.nivel == "SADMIN") || (this.userLevel == 'ADMIN' && this.id_user != fila.id_usuario && fila.nivel == "ADMIN") ? '' : ` 

        
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${index + 1}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${fila.nombre}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${fila.apellido}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${fila.username}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${fila.nivel}</td>
                        
                        <td class="px-4 py-2 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2 justify-center items-center">

                                ${(fila.nivel == 'SADMIN' && fila.id_usuario != this.id_user) || (fila.nivel == 'ADMIN' && this.id_user != fila.id_usuario && this.userLevel == "ADMIN") ? '' : `
                                    <!-- Botón Editar -->
                                ...
                                ${this.id_user == fila.id_usuario || fila.nivel == 'SADMIN' || (fila.nivel == 'ADMIN' && this.userLevel == 'ADMIN') ? '' : `
                                    <!-- Botón Eliminar -->
                                    <button data-id="${fila.id_usuario}"
    ...
    `).join('');
        ...
        if (typeof DataTable !== 'undefined') {
            try {
                this.dataTable = new DataTable('#tabla-usuarios', {
                    language: {
                        url: '../js/es-MX.json'
                    },
                    scrollX: true,
    ...
    async MGuardar(e) {
        ...
        try {
            ...
            const datos = {
                ...
                checkbox: document.getElementById('checkPassword').checked
            };
    ...
    abrirModal(datos = null) {
        document.getElementById('nivel_usr').classList.remove('hidden');
        ...
        if (datos) {
            ...
            document.querySelector('.label_password').classList.remove('hidden');
            document.getElementById('password').setAttribute('readonly', true);

            document.getElementById('tituloModal').textContent = 'Editar Usuario';
            document.getElementById('Id').value = datos.id_usuario;
            ...
            if (datos.nivel == 'SADMIN') {
                document.getElementById('nivel_usr').classList.add('hidden'); //Desparece el select de niveles
                document.getElementById('nivel').removeAttribute('required'); //Quita que sea neseario el select de niveles
            } else {
                document.getElementById('nivel').value = datos.nivel;
                document.getElementById('nivel').setAttribute('required', true);
            }


            if (this.userLevel == "SADMIN") {
                document.querySelector('.selectADMIN').classList.remove('hidden');
            } else {
                document.querySelector('.selectADMIN').classList.add('hidden');

                if (datos.nivel == 'ADMIN') {
                    document.querySelector('.selectADMIN').classList.remove('hidden');
                    document.getElementById('nivel').setAttribute('disabled', true);
                }
            }
            ...
            document.querySelector('.label_password').classList.add('hidden');
    ...
    /* Eliminar */
    MEliminar(id) {
        const elementos = this.elementos.find(e => e.id_usuario == id);
        if (!elementos) {
            this.mostrarToast('No se encontró al usuario a eliminar en el array', 'error');
            return;
        }

        this.editar = id;
        this.elements.nombreEstacionEliminar.textContent = elementos.username || 'Usuario sin Username';

        ...
        <div class="flex justify-between items-center py-6">
            ...
            <div class="flex items-center space-x-4">
                <div>
                    ...
                    <p class="text-gray-500 text-sm">Gestión de Almacen</p>
                </div>
            </div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 mb-8 border border-gray-100">
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center space-y-4 lg:space-y-0">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h2 class="text-2xl font-bold text-gray-900">Registro del Almacen</h2>
                        <p class="text-gray-500 text-sm">Gestiona los elementos del Almacen</p>
                    </div>
                </div>
                    ...
                    <table id="tabla-almacen" class="min-w-full">
                        <thead class="bg-gradient-to-r from-camelsi-blue to-camelsi-dark-blue shadow-lg">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-center text-xs font-medium text-white uppercase tracking-wider">Stock</th>
        ...
        <div id="sinResultados" class="text-center py-12 hidden">
            <div class="animate-bounce-in">
                ...
                <p class="text-gray-500 mb-4">Comienza agregando un nuevo elemento al Almacen.</p>
            </div>
        </div>
        ...
        <div class="modal-content">
            ...
            <form id="formPrincipal" class="space-y-6">
                ...
                <div class="flex flex-wrap gap-6">
                    ...
                    <div class="w-full">
                        <label for="busqueda" class="block text-sm font-semibold text-gray-700 mb-2">Busqueda *</label>
                        <select name="busqueda" id="busqueda" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                            <option selected value="">Mostrar todas</option>
                            <?php
                            try {
                                $sql = "SELECT DISTINCT tipo FROM tipo_articulo";
                                $stmt = $pdo->query($sql);
                                while ($mostrar = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    $tipoCapitalizado = ucwords(strtolower($mostrar['tipo']));
                                    echo "<option value='" . htmlspecialchars($mostrar['tipo'], ENT_QUOTES) . "'>" . $tipoCapitalizado . "</option>";
                                }
                    ...
                    <div class="w-full">
                        <label for="articulo" class="block text-sm font-semibold text-gray-700 mb-2">Articulo *</label>
                        <select name="articulo" id="articulo" required size="1" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                            <option value="" selected hidden disabled>Seleccione una opción</option>
                            <?php
                            try {
                                $sql2 = "SELECT DISTINCT tipo FROM tipo_articulo";
                                $stmt2 = $pdo->query($sql2);

                                while ($mostrar2 = $stmt2->fetch(PDO::FETCH_ASSOC)) {
                                    $tipoMayuscula = ucwords(strtolower($mostrar2['tipo']));
                                    echo "<optgroup class='opto " . htmlspecialchars($mostrar2['tipo'], ENT_QUOTES) . "' label='" . $tipoMayuscula . "'>";

                                    $sql3 = "SELECT * FROM tipo_articulo WHERE tipo = :tipo";
                                    $stmt3 = $pdo->prepare($sql3);
                                    $stmt3->execute([':tipo' => $mostrar2['tipo']]);

                                    while ($mostrar3 = $stmt3->fetch(PDO::FETCH_ASSOC)) {
                                        $tipoMayuscula2 = ucwords(strtolower($mostrar3['nombre']));
                                        echo "<option value='" . htmlspecialchars($mostrar3['id_articulo'], ENT_QUOTES) . "'>" . $tipoMayuscula2 . "</option>";
                                    }
                    ...
                    <div class="w-full">
                        <label for="stock" class="block text-sm font-semibold text-gray-700 mb-2">Agregar Stock *</label>
                        <input type="number" id="stock" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200" required placeholder="1234...">
                    </div>
...
<script src="../../js/JAlmacen.js"></script>
